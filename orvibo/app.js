...
log('Web Site :    http://'+getIPAddress()+':'+PortHttp+"/0/listHTML\n\n", 9);
log('API :  http://'+getIPAddress()+':'+PortHttp+"/{id orvibo}/{command}/{perif}/{blaster name}\n\
\n\
 avec :\n\
...
serverHttp.on('request', function(req, res) {
    ...
    switch (command) {
      ...
      case 'help' :
           buf = fs.readFileSync(__dirname + "/" + "README.MD" ,{ encoding: 'utf8' });
           res.end('help : \n'+ buf);
      break;
...
function listHTML() {
         ...
         var msgSpy ="";
        var obj1 = nconf.get();
        var messageHelp=fs.readFileSync(__dirname + "/" + 'README.MD');
...
//      Fonction de loggues
//
function log(msg, level) { // Shortcut for "console.log". Saves typing when debugging.
    ...
    if(level >= DEBUG_LEVEL) {
        ...
        if(DEBUG_TO_FILE == false) {
            ...
            fs.appendFile(__dirname + "/" + "allone-log.txt", message + "\n", function(err) { });
        }
    }
}
