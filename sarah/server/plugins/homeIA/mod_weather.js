...
exports.do_main = function(words, data, callback) {
	...
	var zipcode = padZip((data.params.zip || pluginProps.zip), 6);
	...
	request({ 'uri' : url }, function (err, response, body) {
		...
		// console.log("Météo zip=" + zipcode + " date=" + data.params.date + " period=" + data.params.period + " (valeur défaut date=" + pluginProps.date + " period=" + false + ")");

		var tts = parseWeather(body, data.params.date || pluginProps.date, data.params.period || false);
		callback({'tts': tts});
  	});
};
